...
use lrgs::{config::{create_lrgs_config, create_managed_users}, configmap::created_script_config_map, controller, service::create_service, state::AppData, statefulset::create_statefulset};
use serde::Serialize;
...
async fn main() ->  anyhow::Result<()> {
    ...
    let context = Arc::new(AppData {
        client: client.clone(),
    });

    let controller = controller::run(context.clone());
...
#[get("/metrics")]
async fn metrics(c: Data<AppData>, _req: HttpRequest) -> impl Responder {
...
#[get("/")]
async fn index(c: Data<AppData>, _req: HttpRequest) -> impl Responder {
